...
def plot_content_over_time(df):
    if df.empty:
        print("Warning: Empty dataframe in plot_content_over_time")
        return go.Figure()
    data = df.groupby(['year_added', 'type']).size().reset_index(name='count')
    fig = px.area(data, x='year_added', y='count', color='type', title='📈 Evolução de Títulos Adicionados por Tipo',
                  color_discrete_sequence=nflix_palette, labels={'year_added': 'Ano', 'count': 'Total de Títulos'},
                  template='plotly_white')
    fig.update_layout(hovermode='x unified', xaxis_title='Ano', yaxis_title='Total de Títulos', legend_title='Tipo')
    return fig

def plot_content_ratio(df):
    if df.empty:
        print("Warning: Empty dataframe in plot_content_ratio")
        return go.Figure()
    ...
    fig.update_layout(title='🎬 Distribuição de Conteúdo', showlegend=False, template='plotly_white')
    return fig

def plot_country_distribution(df, top_n=10):
    if df.empty:
        print("Warning: Empty dataframe in plot_country_distribution")
        return go.Figure()
    ...
    fig.update_layout(title='🌍 Top Países Produtores de Conteúdo', xaxis_title='Número de Títulos',
                      template='plotly_white', plot_bgcolor='rgba(0,0,0,0)', yaxis=dict(tickfont=dict(size=12)))
    return fig

def plot_content_timeline(df):
    if df.empty:
        print("Warning: Empty dataframe in plot_content_timeline")
        return go.Figure()
    ...
    fig.add_vline(x=2016, line=dict(color='gray', dash='dot'), annotation_text="🌍 Expansão Global")
    fig.update_layout(title='📊 Conteúdo Acumulado na Netflix', xaxis_title='Ano',
                      yaxis_title='Total de Conteúdo Adicionado', template='plotly_white',
...
def plot_target_ages_by_country(df):
    if df.empty:
        print("Warning: Empty dataframe in plot_target_ages_by_country")
        return go.Figure()
    ...
    fig = go.Figure(data=go.Heatmap(z=heatmap_data.values, x=heatmap_data.columns, y=heatmap_data.index,
                                    colorscale=[[0, '#ffffff'], [1, nflix_palette[0]]], zmin=0.05, zmax=0.6,
                                    hovertemplate='País: %{x}<br>Faixa Etária: %{y}<br>Proporção: %{z:.1%}<extra></extra>'))
    fig.update_layout(title='🎯 Distribuição por Faixa Etária por País', xaxis_title='País',
                      yaxis_title='Faixa Etária', template='plotly_white', height=500)
    return fig

def plot_release_vs_addition(df, content_type='Movie'):
    if df.empty:
        print("Warning: Empty dataframe in plot_release_vs_addition")
        return go.Figure()
    ...
    fig.update_layout(title=f'📅 {content_type}: Tempo Entre Lançamento e Adição', xaxis_title='Ano',
                      yaxis_title='País', yaxis=dict(tickmode='array', tickvals=list(range(len(df_avg))),
                                                    ticktext=df_avg.index.tolist()), height=600,
...
def plot_time_to_netflix(df):
    if df.empty:
        print("Warning: Empty dataframe in plot_time_to_netflix")
        return go.Figure()
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=('⏱️ Tempo Médio por Ano de Lançamento', 
                                                       '🌍 Tempo Médio por País'))
    fig.add_trace(go.Scatter(x=time_by_year['release_year'], y=time_by_year['time_to_netflix'],
                             mode='lines+markers', name='Tempo Médio', line=dict(color=nflix_palette[0], width=3),
                             marker=dict(size=8)), row=1, col=1)
    fig.add_trace(go.Bar(y=time_by_country['first_country'], x=time_by_country['time_to_netflix'],
                         orientation='h', name='Tempo Médio', marker=dict(color=nflix_palette[0])), row=1, col=2)
    fig.update_layout(title='⏳ Tempo Entre Lançamento e Entrada na Netflix', template='plotly_white',
...
def generate_wordcloud(text):
    wordcloud = WordCloud(width=800, height=400, background_color='black', colormap='Reds',
...
app.layout = dbc.Container([
    dbc.Row([dbc.Col([html.Div([html.H1("📊 Netflix Analytics Dashboard", className='display-4 mb-0'),
                                html.P("Análise Avançada do Catálogo da Netflix", className='lead text-muted'),
                                html.Hr(className="my-3")], className='text-center py-4')])]),
    dbc.Row([
        dbc.Col([
            dbc.Card([dbc.CardHeader("🔍 Filtros Avançados", className='h5'),
                      dbc.CardBody([dbc.Form([html.Div([dbc.Label("Tipo de Título"),
                                                       dcc.Dropdown(id='filter-type', options=type_options,
                                                                    value='Todos', clearable=False)],
                                                     className='mb-3'),
                                             html.Div([dbc.Label("País(es)"),
                                                       dcc.Dropdown(id='filter-country', options=country_options,
                                                                    multi=True, placeholder="Selecione países...")],
                                                     className='mb-3'),
                                             html.Div([dbc.Label("Ano de Lançamento"),
                                             ...
                                             dbc.Button("Limpar Filtros", id="clear-filters", color="danger",
                                                        size="sm", className='w-100 mt-3', outline=True)])])],
                     className='shadow-sm mb-4'),
            dbc.Card([dbc.CardHeader("📊 Indicadores Chave", className='h5'),
                      dbc.CardBody([dbc.Row([dbc.Col(html.Div(id='kpi-total'), width=6),
                                            dbc.Col(html.Div(id='kpi-movies'), width=6)]),
                                    dbc.Row([dbc.Col(html.Div(id='kpi-tv'), width=6),
                                             dbc.Col(html.Div(id='kpi-recent'), width=6)]),
                                    dbc.Row([dbc.Col(html.Div(id='kpi-exclusive'), width=6),
                                             dbc.Col(html.Div(id='kpi-avg-time'), width=6)])])],
                     className='shadow-sm')
        ], md=3),
        dbc.Col([
            dbc.Row([dbc.Col(dcc.Graph(id='content-over-time'), width=6),
                     dbc.Col(dcc.Graph(id='content-ratio'), width=6)], className='mb-4'),
            dbc.Row([dbc.Col(dcc.Graph(id='country-distribution'), width=12)], className='mb-4'),
            dbc.Row([dbc.Col(dcc.Graph(id='time-to-netflix'), width=12)], className='mb-4'),
            dbc.Tabs([dbc.Tab(label="Linha do Tempo", children=[dbc.Row([dbc.Col(dcc.Graph(id='content-timeline'),
                                                                                width=12)], className='mb-4')]),
                      dbc.Tab(label="Faixa Etária", children=[dbc.Row([dbc.Col(dcc.Graph(id='target-ages'),
                                                                              width=12)], className='mb-4')]),
                      dbc.Tab(label="Lançamento vs Adição", children=[dbc.Row([dbc.Col(dcc.Graph(id='release-vs-addition-movie'),
                                                                                      width=6),
                                                                              dbc.Col(dcc.Graph(id='release-vs-addition-tv'),
                                                                                      width=6)], className='mb-4')])]),
            dbc.Row([dbc.Col(dbc.Card([dbc.CardHeader("☁️ Nuvem de Palavras dos Títulos", className='h5'),
                                       dbc.CardBody(html.Img(id='wordcloud-titles', className='img-fluid'))],
                                     className='shadow-sm'), width=12)])
        ], md=9)
    ]),
    dbc.Row([dbc.Col([html.Footer([html.Hr(),
...
# --- Estilos para KPIs ---
kpi_style = {'border-left': f'4px solid {nflix_palette[0]}', 'padding': '10px', 'margin-bottom': '10px',
             'background-color': '#f8f9fa'}

# --- Callback limpar filtros ---
@app.callback([Output('filter-type', 'value'), Output('filter-country', 'value'), Output('filter-year', 'value')],
              Input('clear-filters', 'n_clicks'), prevent_initial_call=True)
def clear_filters(n_clicks):
    return 'Todos', [], [year_min, year_max]

# --- Callback para KPIs ---
@app.callback([Output('kpi-total', 'children'), Output('kpi-movies', 'children'), Output('kpi-tv', 'children'),
               Output('kpi-recent', 'children'), Output('kpi-exclusive', 'children'), Output('kpi-avg-time', 'children')],
              [Input('filter-type', 'value'), Input('filter-country', 'value'), Input('filter-year', 'value')])
def update_kpis(selected_type, selected_countries, year_range):
    dff = df.copy()
    print(f"KPIs - Initial dff shape: {dff.shape}")
    ...
    print(f"KPIs - Filtered dff shape: {dff.shape}")
    ...
    kpi_total = dbc.Card([html.H5(f"{total_titles:,}", className="card-title"),
                          html.P("Total de Títulos", className="card-text text-muted")], style=kpi_style)
    kpi_movies = dbc.Card([html.H5(f"{movies_count:,}", className="card-title"),
                           html.P("Filmes", className="card-text text-muted")], style=kpi_style)
    kpi_tv = dbc.Card([html.H5(f"{tv_count:,}", className="card-title"),
                       html.P("Séries", className="card-text text-muted")], style=kpi_style)
    kpi_recent = dbc.Card([html.H5(f"{recent_content:,}", className="card-title"),
                           html.P("Adicionados desde 2020", className="card-text text-muted")], style=kpi_style)
    kpi_exclusive = dbc.Card([html.H5(f"{exclusive_content:,}", className="card-title"),
                              html.P("Conteúdos Exclusivos", className="card-text text-muted")], style=kpi_style)
    kpi_avg_time = dbc.Card([html.H5(avg_time, className="card-title"),
...
# --- Callback principal ---
@app.callback([Output('content-over-time', 'figure'), Output('content-ratio', 'figure'),
               Output('country-distribution', 'figure'), Output('content-timeline', 'figure'),
               Output('target-ages', 'figure'), Output('release-vs-addition-movie', 'figure'),
               Output('release-vs-addition-tv', 'figure'), Output('wordcloud-titles', 'src'),
               Output('time-to-netflix', 'figure')],
              [Input('filter-type', 'value'), Input('filter-country', 'value'), Input('filter-year', 'value')])
def update_dashboard(selected_type, selected_countries, year_range):
    dff = df.copy()
    print(f"Dashboard - Initial dff shape: {dff.shape}")
    if selected_type != 'Todos':
        dff = dff[dff['type'] == selected_type]
    if selected_countries:
        dff = dff[dff['first_country'].isin(selected_countries)]
    dff = dff[(dff['release_year'] >= year_range[0]) & (dff['release_year'] <= year_range[1])]
    print(f"Dashboard - Filtered dff shape: {dff.shape}")
